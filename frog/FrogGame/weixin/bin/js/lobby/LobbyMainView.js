...
(function (lobby) {
    ...
    var LobbyMainView = /** @class */ (function (_super) {
        ...
        //获取用户信息
        LobbyMainView.prototype.getAuthorize = function () {
            var _this = this;
            utl.ThirdSdk.authorize(function () {
                _this.getUserInfo();
            }, function () {
                var ask = new lobby.TipDialog;
                ask.setTip(def.getLanguageStr(def.LanguageConfig.Keys.OPEN_AUTHORIZE));
                ask.on(ask.EVENTSURE, _this, function () {
                    utl.ThirdSdk.openSetting(function () {
                        _this.getUserInfo();
                    }, function () {
                        var over = new lobby.TipDialog;
                        over.setTip(def.getLanguageStr(def.LanguageConfig.Keys.RESTARTgAME));
                        over.popup(true);
                    });
                });
                ask.popup(true);
        ...
        //获取用户信息，分数
        LobbyMainView.prototype.getUserInfo = function () {
            utl.ThirdSdk.getUserInfo(function (userInfo) {
                def.UserInfo.init(userInfo);
            });
            utl.ThirdSdk.getScore(function (data) {
                // def.UserInfo.setScore(data)
            });
            utl.ThirdSdk.getFriendsInfo(function (data) {
                // def.UserInfo.setFriendsInfo(data)
            });
            //分享监听
            utl.ThirdSdk.shareInit();
        };
